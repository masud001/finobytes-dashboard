...
const FormButton: React.FC<FormButtonProps> = ({
  ...
  const variantClasses = {
    ...
    secondary: 'bg-gray-200 hover:bg-gray-300 focus:ring-gray-500 text-gray-900 border border-gray-300',
  ...
  const hasCustomColors = className.includes('bg-') && className.includes('hover:bg-');
  
  // If custom colors are provided, use them; otherwise use variant colors
  const colorClasses = hasCustomColors ? '' : variantClasses[variant];
